    ...
    try {
        ...
        foreach ($clientes as &$cliente) {
            ...
            if ($cliente['fecha_creacion']) {
                $cliente['fecha_creacion'] = date('d/m/Y', strtotime($cliente['fecha_creacion']));
            }
        }
...
function crearCliente()
{
    ...
    // Validar NIT si se proporciona (8 dígitos)
    if (!empty($nit) && !preg_match('/^[0-9]{8}$/', $nit)) {
        throw new Exception('El NIT debe tener exactamente 8 dígitos numéricos');
    }
    ...
    try {
        ...
        $sql = "INSERT INTO clientes (nombre, telefono, nit, direccion, estado, fecha_creacion) 
                VALUES (?, ?, ?, ?, 'activo', NOW())";

        $stmt = $pdo->prepare($sql);
        $stmt->execute([$nombre, $telefono, $nit, $direccion]);
...
function actualizarCliente()
{
    ...
    try {
        $sql = "UPDATE clientes 
                SET nombre = ?, telefono = ?, nit = ?, direccion = ?
                WHERE id = ? AND estado = 'activo'";

        $stmt = $pdo->prepare($sql);
        $stmt->execute([$nombre, $telefono, $nit, $direccion, $id]);
...
function buscarClientes()
{
    ...
    try {
        $sql = "SELECT 
                    id, nombre, telefono, nit
                FROM clientes 
                WHERE estado = 'activo' 
                AND (nombre LIKE ? OR telefono LIKE ? OR nit LIKE ?)
                ORDER BY nombre ASC
                LIMIT 10";
